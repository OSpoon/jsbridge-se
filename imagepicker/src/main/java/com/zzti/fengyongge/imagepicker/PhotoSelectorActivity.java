...
public class PhotoSelectorActivity extends Activity implements SelectPhotoItem.onItemClickListener, SelectPhotoItem.onPhotoItemCheckedListener,
	...
	private String cropImage(Bitmap cropImage,String path){
		if (cropImage == null) {
			return null;
		}else {
			//存储名称按原名称存储
			String tempPath = new File(path).getName().toString();
			String fileName = tempPath.substring(0,tempPath.lastIndexOf("."));
			File appDir = new File(Environment.getExternalStorageDirectory(), "imagePicker");
			if (!appDir.exists()) {
				appDir.mkdir();
			}
			String path_name = appDir.getAbsolutePath()+File.separator+fileName+".jpg";
			FileUtils.writeImage(cropImage, path_name, 100);
			return path_name;
		}
			...
			new Thread() {
				...
				public void run() {
					for (int i = 0; i < selected.size(); i++) {
						...
						if (degree == 0) {
							cropImage = cropImage(ImageUtils.getimage(PhotoSelectorActivity.this,selected.get(i).getOriginalPath()),selected.get(i).getOriginalPath());
						} else {
							cropImage = cropImage(ImageUtils.rotateBitmapByDegree(ImageUtils.getimage(PhotoSelectorActivity.this,selected.get(i).getOriginalPath()), degree),selected.get(i).getOriginalPath());
						}
...
//			}
	...
	public void onPermissionsDenied(int requestCode, List<String> perms) {
		...
		new AppSettingsDialog.Builder(this)
				...
				.setNegativeButton(getString(R.string.cancel)/* click listener */)
